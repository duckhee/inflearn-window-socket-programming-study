...
int main(int argc, char **argv) {
    ...
    /** 연결 후 동작 에 대한 처리 */
    while ((hClient = ::accept(hSocket, (SOCKADDR *) &clientAddr, &nAddressLength) != INVALID_SOCKET)) {
        puts("새로운 클라이언트 연결이 되었습니다.");
        /** 데이터를 수신*/
        while ((nReceive = ::recv(hClient, pszBuffer, sizeof(pszBuffer), 0))) {
            /** 수신한 데이터 전송 */
            ::send(hClient, pszBuffer, sizeof(pszBuffer), 0);
            /** 수신 받은 데이터 출력 */
            puts(pszBuffer);
            /** 버퍼 초기화 */
